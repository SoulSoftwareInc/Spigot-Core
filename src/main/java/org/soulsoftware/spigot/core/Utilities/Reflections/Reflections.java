...
public class Reflections implements NameHelper {
     ...
     * <p>it is preferred to use {@link ConfigurationBuilder} <pre>{@code new Reflections(new ConfigurationBuilder()...)}</pre>
     */
     ...
     * <p>it is preferred to use {@link ConfigurationBuilder} instead, this is actually similar to:
     ...
     * <p></p><i>it is preferred to use {@link ConfigurationBuilder} instead.</i>
     */
    ...
    /**
     * collect saved Reflections metadata from all urls that contains the given {@code packagePrefix} and matches the given {@code resourceNameFilter},
     * and deserialize using the default serializer {@link XmlSerializer}
     ...
     * <p>{@link Inherited} is not honored by default, see {@link #getTypesAnnotatedWith(Class, boolean)}.
    ...
    /**
     * get types annotated with the given {@code annotation}, both classes and annotations
     * <p>{@link Inherited} is honored according to the given {@code honorInherited}.
     ...
     * <p>{@link Inherited} is not honored by default, see {@link #getTypesAnnotatedWith(Annotation, boolean)}.
     * <p></p><i>depends on {@link Scanners#TypesAnnotated} and {@link Scanners#SubTypes} configured</i>
     */
    ...
    /**
     * returns the {@link Store} object used for storing and querying the metadata
     ...
     * <p></p><i>prefer using a designated directory (for example META-INF/reflections but not just META-INF), so that  can work much faster</i>
     */
